...
function isRateLimited(ip) {
  ...
  const timestamps = requestLog.get(ip).filter(function(t) {
...
function buildEmailHTML(name, company, email, role, message) {
  var timestamp = new Date().toLocaleString('en-ZA', {
  ...
  return [
    ...
    '        </div>',
    '        <a href="mailto:' + email + '?subject=Re:%20Your%20Recruitment%20Enquiry%20%E2%80%94%20Sakeena%20Jacobs%20Recruitment" class="reply-btn">',
...
module.exports = function handler(req, res) {
  ...
  var company = sanitize(body.company || '');
  var email   = sanitize(body.email   || '');
  var role    = sanitize(body.role    || '');
